...
def calculate_xrd(structure_path, output_dir, wavelength="CuKa", symprec=0.1):
    """
    ...
    plt.bar(pattern.x, pattern.y, width=0.5, color='blue', edgecolor='black')
    plt.xlabel(r"$2\theta$ (degrees)")
    plt.ylabel("Intensity (a.u.)")
    plt.title(f"XRD Pattern for {structure.composition.reduced_formula} ({wavelength})")
    plt.grid(True, linestyle='--', alpha=0.7)
    
    plot_path = os.path.join(output_dir, f"{base_name}_xrd.png")
...
if __name__ == "__main__":
    ...
    calculate_xrd(args.structure, args.output_dir, args.wavelength, args.symprec)
