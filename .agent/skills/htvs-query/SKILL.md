...
### Database Querying
- **`setup_htvs_django(settings_module='orgel')`**: Core tool to configure the database environment before running queries.
- **`query_htvs_results(group_name, settings_module, ...)`**: Detailed simulation results (energy, forces, stress).
- **`query_htvs_surfaces_detailed(group_name, settings_module, ...)`**: Surface information with Miller indices.
- **`query_htvs_crystals_detailed(group_name, settings_module, ...)`**: Crystal structures with space groups.
- **`query_htvs_jobs_detailed(group_name, settings_module, ...)`**: Job statuses and timing data.
- **`query_htvs_geoms(group_name, settings_module, formula=None, limit=10)`**: Basic geometry model summaries.
...
- **`save_htvs_structures`**: Import structures from files as Surfaces.
